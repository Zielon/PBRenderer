...
void pbr::MetalMaterial::compute_BxDF(Intersection& intersection, TransportMode mode) const{
	...
	//GOLD
	auto eta_n = glm::vec3(0.13046, 0.48358, 1.5599); // Refractive index
	auto eta_k = glm::vec3(3.7391, 2.2124, 1.9255);   // Extinction coefficient

	intersection.bsdf = std::make_shared<BSDF>(intersection, triangle->scene_object);

	auto fresnel = std::make_shared<FresnelConductor>(eta_n, eta_k);
